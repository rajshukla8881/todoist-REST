...
public class TaskServiceImplementer implements TaskService {
    ...
    @Override
    public List<ActiveTaskResponse> getActiveTasks(String username) {
        List<ActiveTaskResponse> activeTaskRequestList = new ArrayList<>();

        //get google events
        try {
            activeTaskRequestList.addAll(GoogleCalenderService.getUserEvents(username));
        } catch (IOException | GeneralSecurityException e) {
            // noop proceed with the todoist events
        }

        //get todoist events
        List<Task> taskList = taskRepository.getTasklistByUsername(username);
    ...
    @Override
    public CreateTaskResponse createTask(TaskRequest taskRequest, String username) {
        Task task = new Task();
        ...
        if (projectRepository.existsById(taskRequest.getProject_id()))
            task.setProject_id(taskRequest.getProject_id());
        else
            task.setProject_id(0);

        //Set Section Id
        if (sectionRepository.existsById(taskRequest.getSection_id()))
            task.setSection_id(taskRequest.getSection_id());
        else
            task.setSection_id(0);
        ...
        CreateTaskResponse createTaskResponse = CreateTaskResponse.builder()
                ...
                .project_id(saveTask.getProject_id())
                .section_id(saveTask.getSection_id())
                .url(saveTask.getUrl())
                .build();
